import Imagescard from "@/components/propertyImagescard";
...
const PropertyPage = async ({ params }) => {
	...
	return (
		<>
			...
			<Link
				href="/properties"
				className="flex items-center gap-2 text-blue-600 text-lg hover:text-black mt-4"
			>
					...
					<div className="grid grid-cols-1 md:grid-cols-70/30 w-full gap-6">
						...
						<main>
									...
									<div className="text-3xl  py-3 px-6 flex justify-center gap-6 mb-6 ">
										<div className=" flex-col items-center text-center">
											...
											{property.rates.nightly ? (
												<p className=" text-blue-600 bg-orange-100 font-boldrounded-xl rounded-xl shadow-md ">
													{" "}
													<FaDollarSign className="inline-block mr-1" />
													{getPrice({ nightly: property.rates.nightly })}{" "}
												</p>
											) : (
										...
										<div className=" flex-col items-center text-center">
											...
											{property.rates.weekly ? (
												<p className=" text-blue-600 bg-orange-100 font-boldrounded-xl  rounded-xl  shadow-md ">
													{" "}
													<FaDollarSign className="inline-block mr-1" />{" "}
													{getPrice({ weekly: property.rates.weekly })}{" "}
												</p>
											) : (
												<FaTimes className="text-red-500" />
											)}{" "}
										...
										<div className=" flex-col items-center text-center">
											...
											{property.rates.monthly ? (
												<p className=" text-blue-600 bg-orange-100 font-boldrounded-xl  rounded-xl shadow-md ">
													{" "}
													<FaDollarSign className="inline-block mr-1" />{" "}
													{getPrice({ monthly: property.rates.monthly })}{" "}
												</p>
											) : (
												<FaTimes className="text-red-500 " />
											)}{" "}
							...
							<Card className="mb-6">
								...
								<CardContent>
									<Imagescard images={property.images} />
								</CardContent>
							</Card>
						</main>
