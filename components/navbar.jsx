...
const Navbar = () => {
	...
	// const [isLoggedIn, setIsLoggedIn] = useState(false); //temp hardcoded logged in state that we will replace with next auth session

	const [providers, setProviders] = useState(null);
	useEffect(() => {
		const setAuthProviders = async () => {
			const res = await getProviders();
			setProviders(res); // set the providers in state to use later for sign in buttons to show different providers like google for now , if configured others will show too
		};
		setAuthProviders(); // call the function to set providers when component mounts (only once)
	}, []);
				...
				<div className="relative flex h-20 items-center justify-between">
					<div className="absolute inset-y-0 left-0 flex items-center md:hidden">
						...
						<button
							...
							className="relative flex items-center justify-center rounded-md p-2 text-white hover:bg-gray-700 hover:text-white
							...
							<div className="flex items-center">
								{providers &&
									Object.values(providers).map(
										(
											provider,
											index, //map through providers object values to get each provider like google
										) => (
											<button
												key={index}
												onClick={() => signIn(provider.id)} //call sign in with provider id on click
									...
									>
										...
										<button
											...
											onClick={() => {
												...
												signOut();
											}} //call sign out on click and close the dropdown menu
										>
											Sign Out
										</button>
									</div>
